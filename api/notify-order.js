...
module.exports = async function handler(req, res) {
  ...
  // ── Recipients
  // Admin emails: from request first, then Firebase, then env var fallback
  let resolvedEmails = Array.isArray(notifEmails) && notifEmails.length ? notifEmails : null;
  let resolvedPhones = Array.isArray(notifPhones) && notifPhones.length ? notifPhones : null;

  // If not passed from client, try Firebase
  if (!resolvedEmails || !resolvedPhones) {
    try {
      ...
      if (snap.exists()) {
        ...
        if (!resolvedEmails && notifData.emails?.length) resolvedEmails = notifData.emails;
        if (!resolvedPhones && notifData.phones?.length) resolvedPhones = notifData.phones;
        console.log('✅ Notif settings loaded from Firebase:', resolvedEmails);
      }
    } catch(e) {
      console.warn('Firebase notif settings load failed (using fallback):', e.message);
    }
  }
  ...
  try {
    ...
    console.log('Admin email status:', emailRes.status, emailBody || 'OK');
  ...
  if (guestEmail) {
    try {
      ...
      console.log('Guest email status:', guestRes.status, guestBody || 'OK');
