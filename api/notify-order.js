...
module.exports = async function handler(req, res) {
  ...
  if (req.body.isSaveFailureAlert) {
    ...
    const adminEmails = (notifEmails && notifEmails.length ? notifEmails : ['yshahn@gmail.com', 'pacificrimbistro@gmail.com']);
    ...
    try {
      await sendEmail({
        to: adminEmails,
        subject: `⚠️ Order NOT saved to dashboard - Pacific Rim Bistro`,
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
    ...
    if (guestPhone) {
      ...
      // message registered with Telnyx's 10DLC campaign. The link-based
      // Yes/No flow (shortYes/shortNo) is kept for email only; email isn't
      // subject to carrier SMS content rules.
