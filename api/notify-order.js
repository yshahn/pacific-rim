...
Phone: ${custPhone}${orderId ? '\nOrder #' + orderId : ''}`;
  ...
  // Handle various newline escape formats from Vercel env vars
  const fcmPrivateKeyRaw = process.env.FCM_PRIVATE_KEY || '';
  // Vercel may store the key with literal \n or with actual newlines
  // We need actual newlines for the crypto module to parse it
  let fcmPrivateKey = fcmPrivateKeyRaw;
  if (!fcmPrivateKey.includes('\n')) {
    // No real newlines — replace literal \n with real newlines
    fcmPrivateKey = fcmPrivateKey.replace(/\\n/g, '\n');
  }
  fcmPrivateKey = fcmPrivateKey.trim();
  const fcmProjectId   = process.env.FCM_PROJECT_ID || 'pacific-rim-bistro';
  console.log('📲 FCM key check: hasBegin=', fcmPrivateKey.includes('BEGIN PRIVATE KEY'), 'hasNewline=', fcmPrivateKey.includes('\n'), 'len=', fcmPrivateKey.length);
