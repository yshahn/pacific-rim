...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  ...
  if (isReservation) {
    ...
    // SMS is kept short and emoji-free on purpose: emoji forces UCS-2
    // encoding (70 chars/segment instead of 160), and long itemized orders
    // (10+ items) were exceeding the 10-segment carrier limit — AT&T was
    // rejecting these outright while other carriers happened to let them
    // through. Full itemized details still go out via email (below) and
    // are already visible in the admin dashboard/push notification, so
    // nothing is lost — the SMS is just a short heads-up now.
    ...
    guestSmsText = [
      ...
      specialReq ? `Note: ${specialReq}` : '',
      `Full receipt sent to your email. Questions? (404) 893-0018${arrivedLink}`,
    ].filter(Boolean).join('\n');
