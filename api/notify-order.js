      ...
      for (const toPhone of adminPhones) {
        try {
          const r = await fetch(twilioUrl, {
            ...
            body: new URLSearchParams({ MessagingServiceSid: 'MGf7ff8e0d7feee0f6d48d5e683fdbb38e', To: toPhone, Body: smsText }),
          });
    ...
    for (const toPhone of adminPhones) {
      try {
        const r = await fetch(twilioUrl, {
          method: 'POST',
          headers: { 'Authorization': twilioAuth, 'Content-Type': 'application/x-www-form-urlencoded' },
          body: new URLSearchParams({ MessagingServiceSid: 'MGf7ff8e0d7feee0f6d48d5e683fdbb38e', To: toPhone, Body: adminSmsText }),
        });
    ...
    if (guestPhone && guestPhone !== '—' && req.body.guestSmsConsent) {
      try {
        const r = await fetch(twilioUrl, {
          method: 'POST',
          headers: { 'Authorization': twilioAuth, 'Content-Type': 'application/x-www-form-urlencoded' },
          body: new URLSearchParams({ MessagingServiceSid: 'MGf7ff8e0d7feee0f6d48d5e683fdbb38e', To: guestPhone, Body: guestSmsText }),
        });
        const d = await r.json().catch(() => ({}));
