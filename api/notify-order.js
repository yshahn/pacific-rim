// ── Order Notification API (CommonJS)
...
module.exports = async function handler(req, res) {
  ...
  const { orderItems, subtotal, tax, tip, total, customer, orderId, pickupTime, isReservation, special } = req.body;
  ...
  const phone     = customer?.phone ? (customer.phone.startsWith('+') ? customer.phone : '+1' + customer.phone.replace(/\D/g,'')) : '—';
  const orderCount = orderItems.length;

  // ── RESERVATION EMAIL
  if (isReservation) {
    const emailText = `New Reservation — Pacific Rim Bistro

Guest: ${customer?.name || '—'}
Date/Time: ${pickup}
Party Size: ${orderItems[0]?.name || '—'}
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
    <div style="text-align:center;margin-bottom:24px;">
      ...
      <h1 style="font-size:24px;margin:8px 0 4px;color:#0f0e0c;">New Reservation!</h1>
    ...
    <div style="background:#f5f2ec;border-radius:10px;padding:16px;margin-bottom:20px;">
      <p style="margin:0;font-size:16px;font-weight:600;">${customer?.name || '—'}</p>
      <p style="margin:4px 0 0;font-size:20px;color:#c8a96e;font-weight:600;">${pickup}</p>
      <p style="margin:4px 0 0;font-size:13px;color:#888;">${orderItems[0]?.name || ''}</p>
    </div>
    <table style="width:100%;font-size:14px;margin-bottom:20px;">
      ...
      <tr><td style="padding:5px 0;color:#888;">Email</td><td style="font-weight:500;">${customer?.email || '—'}</td></tr>
    ...
    try {
      const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
        ...
        body: JSON.stringify({
          ...
          from: { email: 'yshahn@gmail.com', name: 'Pacific Rim Bistro' },
          reply_to: { email: 'pacificrimbistro@gmail.com' },
          subject: `🗓️ New Reservation — ${customer?.name || 'Guest'} · ${pickup}`,
...
Requested pick up time: ${pickup}

The payment has been authorized using the credit card entered. You need to accept the order within the next 7 days for the payment to go through.
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
    <div style="text-align:center;margin-bottom:24px;">
      ...
      <h1 style="font-size:24px;margin:8px 0 4px;color:#0f0e0c;">Congrats!</h1>
    ...
    <h3 style="font-size:13px;text-transform:uppercase;color:#888;margin:0 0 10px;">Request Details</h3>
    <table style="width:100%;font-size:14px;margin-bottom:20px;">
      <tr><td style="padding:5px 0;color:#888;width:130px;">First name</td><td style="font-weight:500;">${firstName}</td></tr>
      <tr><td style="padding:5px 0;color:#888;">Last name</td><td style="font-weight:500;">${lastName}</td></tr>
      <tr><td style="padding:5px 0;color:#888;">Phone</td><td style="font-weight:500;">${phone}</td></tr>
    ...
    <h3 style="font-size:13px;text-transform:uppercase;color:#888;margin:0 0 10px;">Order Details</h3>
    <div style="border:1px solid #eee;border-radius:8px;overflow:hidden;margin-bottom:16px;">
      ${orderItems.map((item,i) => `<div style="padding:10px 16px;${i<orderItems.length-1?'border-bottom:1px solid #eee;':''}display:flex;justify-content:space-between;">
    ...
    <table style="width:100%;font-size:14px;margin-bottom:16px;">
      ...
      <tr style="border-top:2px solid #c8a96e;">
        <td style="padding:10px 0 4px;font-weight:700;font-size:15px;">Order size: ${orderCount} item${orderCount!==1?'s':''}</td>
        <td style="text-align:right;font-weight:700;font-size:15px;color:#c8a96e;padding-top:10px;">$${(total||0).toFixed(2)}</td>
      </tr>
    </table>
    <div style="background:#f0f7ff;border-left:4px solid #c8a96e;padding:14px 16px;border-radius:4px;font-size:13px;color:#555;line-height:1.6;margin-bottom:20px;">
      📦 <strong>Requested Pickup Time: ${pickup}</strong><br><br>
      The payment has been authorized. Accept within 7 days for payment to process.
    </div>
    <div style="text-align:center;color:#bbb;font-size:12px;border-top:1px solid #eee;padding-top:16px;">
      Pacific Rim Bistro · 303 Peachtree Center Ave, Atlanta, GA 30303<br>
  ...
  try {
    const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        reply_to: { email: 'pacificrimbistro@gmail.com' },
        subject: `🎉 New Order — ${customer?.name || 'Guest'} · $${(total||0).toFixed(2)} · ${pickup}`,
        content: [
