...
Phone: ${custPhone}${orderId ? '\nOrder #' + orderId : ''}`;
  ...
  if (fcmServerKey) {
    try {
      ...
      const tokenSnap = await fetch(
        `https://firestore.googleapis.com/v1/projects/pacific-rim-bistro/databases/(default)/documents/push_tokens`,
        { headers: { 'Authorization': 'Bearer ' + fcmServerKey } }
      );

      if (tokenSnap.ok) {
        const tokenData = await tokenSnap.json();
        const tokens = (tokenData.documents || []).map(d => d.fields?.token?.stringValue).filter(Boolean);

        // Admin tokens (role = admin in notifEmails)
        const guestEmail = req.body.customerEmail || req.body.customer?.email;
        const adminTokens = (tokenData.documents || [])
          .filter(d => adminEmails.includes(d.fields?.email?.stringValue))
          .map(d => d.fields?.token?.stringValue).filter(Boolean);
        const guestTokens = guestEmail
          ? (tokenData.documents || [])
              .filter(d => d.fields?.email?.stringValue === guestEmail)
              .map(d => d.fields?.token?.stringValue).filter(Boolean)
          : [];
        ...
        const sendPush = async (token, title, body) => {
          await fetch('https://fcm.googleapis.com/fcm/send', {
            ...
            body: JSON.stringify({ to: token, notification: { title, body, icon: '/icons/icon-192.png' } })
          });
        };

        const adminPushTitle = isArrived ? `🚗 ${custName || 'Guest'} Arrived!` : `🍣 New Order — $${(total||0).toFixed(2)}`;
        const adminPushBody  = isArrived ? `Vehicle: ${carInfo}` : `${(orderItems||[]).length} items · ${pickup}`;
        for (const t of adminTokens) await sendPush(t, adminPushTitle, adminPushBody).catch(() => {});

        if (!isArrived && guestTokens.length > 0) {
          const guestTitle = `✅ Order Confirmed — Pacific Rim Bistro`;
          const guestBody  = `Pickup: ${pickup} · $${(total||0).toFixed(2)}`;
          for (const t of guestTokens) await sendPush(t, guestTitle, guestBody).catch(() => {});
        }

        results.push = { adminTokens: adminTokens.length, guestTokens: guestTokens.length };
      }
