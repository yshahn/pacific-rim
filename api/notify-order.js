...
module.exports = async function handler(req, res) {
  ...
  if (isReservation) {
    ...
    const arrivedLink = isCurbside
      ? `\n\n🚗 Curbside Pickup — tap when you arrive:\nhttps://pacificrimatl.com?arrived=1&order=${encodeURIComponent(orderId)}`
      : '';
    const guestItemLines = orderItems.map(i => `  ${i.emoji||'•'} ${i.name} · $${parseFloat(i.price||0).toFixed(2)}`).join('\n');
    const guestPtsTxt = (parseFloat(pointsDiscount)||0) > 0 ? `\n  ✦ Rewards: -$${parseFloat(pointsDiscount).toFixed(2)}` : '';
    const tipAmt = parseFloat(req.body.tip) || 0;
    const tipLine = tipAmt > 0 ? `\n  Tip: $${tipAmt.toFixed(2)}` : '';
    guestSmsText = [
