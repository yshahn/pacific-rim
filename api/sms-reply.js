...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // This Messaging Profile's webhook fires for BOTH outbound (messages we
    // send) and inbound (guest replies) events. Only inbound guest replies
    // should ever be processed here — outbound events have direction
    // "outbound-api"/"outbound" and event_type "message.sent", and their
    // "from" is our own Telnyx number, not a guest's phone. Skipping these
    // early avoids unnecessary Firestore lookups on every message we send.
    if (eventType && eventType !== 'message.received') {
      return res.status(200).json({ received: true, handled: false, reason: 'not_inbound_event' });
    }
    if (direction && direction !== 'inbound') {
      return res.status(200).json({ received: true, handled: false, reason: 'not_inbound_direction' });
    }

    const fromPhone = payload.from?.phone_number || payload.from || '';
    const text = payload.text || '';
    console.log('[sms-reply] inbound from=', fromPhone, 'text=', text);
    ...
    // ── Search each restaurant's Firestore for a phone match with
    // confirmationSent=true. If more than one restaurant has a matching
    // reservation for this phone number (e.g. the same person has a
    // pending confirmation at two restaurants at once), prefer whichever
    // one most recently sent its confirmation SMS — that's the one the
    // guest is almost certainly replying to — rather than picking based on
    // the fixed order of the RESTAURANTS list below.
    const allMatches = [];
      ...
      try {
        ...
        snap.forEach(d => {
          ...
          if (dataDigits.endsWith(fromDigits.slice(-10)) || fromDigits.endsWith(dataDigits.slice(-10))) {
            allMatches.push({ restaurant: r, db, reservation: { ...data, id: d.id } }); // id LAST, see note in reservation object below
          }
        });
      } catch(e) {
    ...
    // Prefer the match with the most recent confirmationSentAt (fall back
    // to createdAt if that's ever missing on an older record).
    allMatches.sort((a, b) =>
      (b.reservation.confirmationSentAt || b.reservation.createdAt || 0) -
      (a.reservation.confirmationSentAt || a.reservation.createdAt || 0)
    );
    const best = allMatches[0];
    if (allMatches.length > 1) {
      console.warn('[sms-reply] Ambiguous match — phone has pending confirmations at multiple restaurants:',
        allMatches.map(m => `${m.restaurant.key}:${m.reservation.id}`).join(', '),
        '→ chose', best.restaurant.key);
    }

    const matchRestaurant = best?.restaurant || null;
    const matchReservation = best?.reservation || null;
    const matchDb = best?.db || null;
    ...
    // Nothing previously checked whether the reservation date had already
    // gone by before processing a "1"/"2" reply — a guest replying days
    // late (e.g. they missed the original text and only just saw it) would
    // still get treated as confirming/cancelling an upcoming reservation,
    // and the restaurant would get a normal-looking "CONFIRMED" alert for
    // a date that's already come and gone. Parse the stored date (e.g.
    // "Sep 14, 2026") and compare by calendar day, not exact time — a late
    // reply on the SAME day the reservation was for is still fine.
    // Per Yong: a reply to an already-passed reservation should produce NO
    // text to anyone — not the guest, not the restaurant — it's moot, so
    // this is a fully silent no-op rather than sending an "already passed"
    // notice back to the guest.
    const resDateParsed = new Date(matchReservation.date);
    if (!isNaN(resDateParsed.getTime())) {
      const todayStart = new Date(); todayStart.setHours(0, 0, 0, 0);
      if (resDateParsed.getTime() < todayStart.getTime()) {
        console.warn('[sms-reply] Reply for a past reservation date, ignoring silently:', matchReservation.date, 'restaurant:', matchRestaurant.key);
        return res.status(200).json({ received: true, handled: true, ignoredPastDate: true, restaurant: matchRestaurant.key });
      }
    }
