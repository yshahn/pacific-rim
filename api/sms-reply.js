  ...
  try {
    ...
    if (!isNaN(resDateParsed.getTime())) {
      ...
      if (resDateParsed.getTime() < todayStart.getTime()) {
        console.warn('[sms-reply] Reply for a past reservation date, ignoring as actionable:', matchReservation.date, 'restaurant:', matchRestaurant.key);
        await sendSMS(fromPhone,
          `Thanks for the reply, but that reservation (${matchReservation.date}) has already passed. ` +
          `If you'd like to book a new reservation, please call us at ${matchRestaurant.phone} or visit our website.`
        );
