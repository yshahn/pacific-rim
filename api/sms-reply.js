...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // NOTE: filtering confirmationSent in JS rather than via a Firestore
    // `where` clause combined with `orderBy` on a different field — that
    // combination requires a composite index to exist in Firestore, and
    // without it the query throws (silently, from the guest's point of
    // view, since this whole handler is wrapped in try/catch). Ordering by
    // a single field (createdAt) needs no special index.
    ...
    snap.forEach(d => {
      ...
      if (digitsOnly(data.phone).endsWith(fromDigits.slice(-10)) || fromDigits.endsWith(digitsOnly(data.phone).slice(-10))) {
        // IMPORTANT: `id: d.id` must come AFTER `...data`, not before.
        // Reservation documents store their own internal `id` field as
        // part of the data (set when the reservation was created) — if
        // that field comes second in the spread, it silently overwrites
        // the real Firestore document ID (d.id) with that internal value,
        // and every later doc(db, 'reservations', match.id) call points at
        // a document that doesn't exist, causing the transaction update to
        // fail with "Can't update a document that doesn't exist."
        match = { ...data, id: d.id };
      }
    });
