...
module.exports = async function handler(req, res) {
  ...
  // This is the EMAIL-LINK confirmation path ("Yes, I'm Coming" / "No,
  // Please Cancel") — a separate path from the SMS "Reply 1/2" flow in
  // api/sms-reply.js, which already has this same check. A guest clicking
  // an old confirmation-request email days after the reservation date has
  // passed should produce NO notification to anyone, the same as a late
  // SMS reply — it's moot. Without this, a stale link click still fired a
  // normal-looking "CONFIRMED" alert for a reservation that already came
  // and went.
...
<title>Reservation Already Passed — Pacific Rim Bistro</title>
<style>
  ...
  h1 { font-size: 22px; color: #555; margin: 0 0 12px; }
  p { font-size: 16px; color: #555; line-height: 1.6; margin: 0 0 8px; }
...
<body>
  <div class="card">
    <div class="emoji">📅</div>
    <h1>This Reservation Has Passed</h1>
    <p>If you'd like to book a new one, please call us at <span class="phone">(404) 893-0018</span> or visit our website.</p>
  </div>
</body></html>`);
    }
  }
  ...
  //     Treating a Firebase error as "not yet processed" used to send a
  //     notification anyway — so the more Firebase struggled, the MORE
  //     duplicate notifications went out. That's backwards.
  ...
  // Skip admin email/SMS/push entirely if this exact response was already
  // recorded recently — avoids duplicate notifications from link-scanning bots.
  if (shouldNotify) {
