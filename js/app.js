...
function goTo(id, pushState = true) {
  ...
  updateNav(id);
  window.scrollTo(0, 0);
  // Android back button support — keep URL clean
...
async function sendArrivedMessage() {
  ...
  if (!order) return;
  const btn = document.querySelector('[onclick="sendArrivedMessage()"]');
  ...
  try {
    ...
    if (btn) { btn.textContent = '✅ Restaurant Notified!'; btn.style.background = '#2ecc71'; }
    ...
    if (statusEl) statusEl.textContent = 'Please try again or call us.';
  }
}
...
function buildMenu(tab) {
  ...
  items.forEach((item, idx) => {
    ...
    // ── Lunch Bento Box accordion only
    if (tab === 'lunch') {
      const BENTO_ITEMS = ['Sweet and Sour Fish','Black Pepper Beef','Fresh Ginger Chicken','Garlic Shrimp','Mongolian Beef','Thai Yellow Curry (Bento)','Green Avocado Curry (Bento)','Panang Curry (Bento)','Spicy Thai Basil (Bento)','Spicy Seven Flavored Beef (Bento)','Wanchi Shrimp (Bento)','Grilled Teriyaki Chicken (Bento)','Grilled Bangkok Chicken (Bento)'];

      // Accordion sections config
      const ACCORDION_CONFIG = [
        { key:'bento',    firstItem:'Sweet and Sour Fish',        emoji:'🍱', label:'Bento Box Specials',  sub:'Served with Miso Soup, House Salad, Spring Roll and Steamed Rice', items: BENTO_ITEMS },
        { key:'classic',  firstItem:'Lunch California Roll',       emoji:'🍣', label:'House Classic Roll',  sub:'', items: [] },
        { key:'special',  firstItem:'Lunch Firecracker Roll',      emoji:'🍣', label:'House Special Roll',  sub:'', items: [] },
        { key:'chef',     firstItem:'Lunch Playboy on Fire Roll',  emoji:'🍣', label:'Chef Special Roll',   sub:'', items: [] },
      ];

      // Dynamically group items per section
      const lunchItems = window.firebaseMenuData?.lunch || menuData.lunch || [];
      const classicStart = lunchItems.findIndex(i => i.n === 'Lunch California Roll');
      const specialStart = lunchItems.findIndex(i => i.n === 'Lunch Firecracker Roll');
      const chefStart    = lunchItems.findIndex(i => i.n === 'Lunch Playboy on Fire Roll');
      ACCORDION_CONFIG[1].items = lunchItems.slice(classicStart, specialStart).map(i => i.n);
      ACCORDION_CONFIG[2].items = lunchItems.slice(specialStart, chefStart).map(i => i.n);
      ACCORDION_CONFIG[3].items = lunchItems.slice(chefStart).map(i => i.n);

      // Find which section this item belongs to
      const mySection = ACCORDION_CONFIG.find(s => s.firstItem === item.n || (s.items.includes(item.n) && s.firstItem !== item.n));
      const isFirstOfSection = ACCORDION_CONFIG.find(s => s.firstItem === item.n);

      if (isFirstOfSection) {
        const sec = isFirstOfSection;
        const isOpen = window._lunchAccordion[sec.key] || false;
        ...
        accHeader.innerHTML =
          '<div class="mi-main" style="width:100%;pointer-events:none;">' +
            '<div class="mi-emoji">' + sec.emoji + '</div>' +
            '<div class="mi-info">' +
              '<div class="mi-name">' + sec.label + ' <span style="color:var(--gold);">→</span></div>' +
              (sec.sub ? '<div class="mi-desc">' + sec.sub + '</div>' : '') +
            '</div>' +
        ...
        accHeader.addEventListener('click', () => {
          window._lunchAccordion[sec.key] = !window._lunchAccordion[sec.key];
          buildMenu('lunch');
        });
        list.appendChild(accHeader);
        if (!isOpen) return;
      } else if (mySection && !window._lunchAccordion[mySection.key]) {
        return; // hide items when section collapsed
      }
    }
    ...
    if (item.toppings && item.toppings.length) {
      ...
      if (qtyCtrl) {
        qtyCtrl.querySelector('.mi-qty-plus').addEventListener('click', () => openToppingModal(item));
        qtyCtrl.querySelector('.mi-qty-minus').addEventListener('click', () => {
          const lastIdx = [...cart].map((c,i)=>[c,i]).filter(([c])=>c.name===item.n).pop()?.[1];
          if (lastIdx !== undefined) cart.splice(lastIdx, 1);
          updateCartBar();
...
// ── Build guest menu tabs dynamically from Firebase categories ──
window.buildGuestMenuTabs = function buildGuestMenuTabs(cats) {
...
function buildPickupSlots() {
  ...
  if (s.allowSchedule !== false) {
    ...
    while (t <= closeTime && t.getTime() - now.getTime() <= maxMs) {
      ...
      // Skip 2:00PM (14:00) to 4:59PM (16:59) and 9:00PM (21:00) onwards
      const skip = (totalMins >= 14 * 60 && totalMins < 17 * 60) || totalMins >= 21 * 60;
      if (!skip) {
...
function getTierPoints(user) {
  // Use tierPoints for tier calculation (resets Jan 1), but total points for balance
...
// Check yearly reset (Jan 1) - tier resets to Bronze, points are kept
function checkYearlyReset() {
  const user = getUser();
  if (!user) return;
  const now = new Date();
  const lastReset = user.lastReset ? new Date(user.lastReset) : null;
  const isNewYear = !lastReset || lastReset.getFullYear() < now.getFullYear();
  if (isNewYear && now.getMonth() === 0 && now.getDate() === 1) {
    user.tierPoints = 0; // reset tier calculation points only
    user.pointsLog = user.pointsLog || [];
    user.pointsLog.push({ desc: 'Annual Tier Reset → Bronze', pts: 0, date: Date.now() });
...
function buildRewardsScreen() {
  ...
  if (rhSub)    rhSub.textContent    = user ? tier.icon + ' ' + tier.name + ' · ' + tier.rate + ' pts per $1' : 'Sign in to view your points';
  ...
  if (params.get('payment') === 'success') {
    ...
    if (pendingOrder) {
      try {
        ...
        // 3. Update points - deduct used, add earned
        try {
          const user = getUser();
          if (user) {
            const earnRate = getEarnRate(user);
            const earned = Math.floor((orderData.subtotal || 0) * earnRate);
            const used = orderData.pointsDiscount ? orderData.pointsDiscount * 100 : 0;
            user.points = Math.max(0, (user.points || 0) - used + earned);
            user.tierPoints = (user.tierPoints || 0) + earned;
            user.pointsLog = user.pointsLog || [];
            if (used > 0) user.pointsLog.push({ desc: 'Points redeemed ($' + orderData.pointsDiscount + ' off)', pts: -used, date: Date.now() });
            user.pointsLog.push({ desc: 'Order #' + (orderData.orderId || ''), pts: earned, date: Date.now() });
            localStorage.setItem(USER_KEY, JSON.stringify(user));
            console.log('🔍 Points updated! earned:', earned, 'used:', used, 'new total:', user.points);
            // Save points to Firebase immediately
            const fbUrlPts = window.location.origin + '/js/firebase-menu.js';
            import(fbUrlPts).then(({ savePointsToFirebase }) => {
              savePointsToFirebase(user.email, user.points, user.pointsLog, user.tierPoints).catch(() => {});
            }).catch(() => {});
            document.getElementById('earned-pts').textContent = '+' + earned + ' pts earned';
            const earnedBadge = document.querySelector('.earned-badge');
            if (earnedBadge) earnedBadge.style.display = 'block';
            updateAuthUI();
          }
...
document.addEventListener('DOMContentLoaded', () => {
  // Load profile from Firebase on init
  const _initUser = getUser();
  if (_initUser?.email) {
    import(window.location.origin + '/js/firebase-menu.js').then(({ loadPointsFromFirebase, loadProfileFromFirebase }) => {
      // Load points from Firebase (source of truth for all devices)
      loadPointsFromFirebase(_initUser.email).then(fbPts => {
        if (fbPts) {
          ...
          if (user) {
            ...
            user.tierPoints = fbPts.tierPoints;
            localStorage.setItem(USER_KEY, JSON.stringify(user));
            updateAuthUI();
          }
        }
      ...
      // Load profile info (skip on payment return to avoid timing issues)
...
function openToppingModal(item) {
  ...
  document.getElementById('topping-qty').textContent = '1';
  ...
  body.innerHTML =
    `<label style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid #f0ebe2;cursor:pointer;">
      <input type="radio" name="topping-choice" value="none" checked
        style="width:20px;height:20px;accent-color:var(--gold);cursor:pointer;flex-shrink:0;" onchange="updateToppingConfirmBtn()"/>
      <span style="font-size:18px;">🚫</span>
      <span style="flex:1;font-size:14px;font-weight:500;">No Topping</span>
    </label>` +
    item.toppings.map((t, ti) => `
    <label style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid #f0ebe2;cursor:pointer;">
      <input type="checkbox" data-idx="${ti}" name="topping-choice"
        style="width:20px;height:20px;accent-color:var(--gold);cursor:pointer;flex-shrink:0;" onchange="updateToppingConfirmBtn()"/>
  ...
  body.querySelectorAll('input[type="checkbox"]').forEach(chk => {
    chk.addEventListener('change', () => {
      const noTopping = body.querySelector('input[type="radio"]');
      if (chk.checked && noTopping) noTopping.checked = false;
      const anyChecked = [...body.querySelectorAll('input[type="checkbox"]')].some(c => c.checked);
      if (!anyChecked && noTopping) noTopping.checked = true;
      updateToppingConfirmBtn();
    });
  });
  const noToppingRadio = body.querySelector('input[type="radio"]');
  if (noToppingRadio) {
    noToppingRadio.addEventListener('change', () => {
...
function updateToppingConfirmBtn() {
  ...
  const total = (parseFloat(_toppingItem.p) + toppingTotal) * qty;
  btn.textContent = `Add ${qty > 1 ? qty + 'x ' : ''}to Cart — $${total.toFixed(2)}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const _initUser = getUser();
  if (_initUser?.email) {
    import(window.location.origin + '/js/firebase-menu.js').then(({ loadProfileFromFirebase }) => {
      loadProfileFromFirebase(_initUser.email).then(fbProfile => {
        if (fbProfile) {
          // Don't overwrite points if local is newer (e.g. just after payment)
          const localPoints = _initUser.points || 0;
          const fbPoints    = fbProfile.points || 0;
          const merged = {
            ..._initUser,
            firstName: fbProfile.firstName || _initUser.firstName || '',
            lastName:  fbProfile.lastName  || _initUser.lastName  || '',
            phone:     fbProfile.phone     || _initUser.phone     || '',
            address:   fbProfile.address   || '',
            city:      fbProfile.city      || '',
            state:     fbProfile.state     || '',
            zip:       fbProfile.zip       || '',
            points:    Math.max(localPoints, fbPoints),
            pointsLog: (localPoints >= fbPoints ? _initUser.pointsLog : fbProfile.pointsLog) || [],
          };
          // Only update localStorage if Firebase has newer profile info (not points)
          const profileChanged = fbProfile.firstName !== _initUser.firstName ||
                                 fbProfile.phone !== _initUser.phone;
          if (profileChanged) {
            merged.points    = localPoints; // always keep local points
            merged.pointsLog = _initUser.pointsLog || [];
  ...
  try {
    ...
    myOrders.forEach(order => {
      ...
      card.innerHTML =
        ...
        '</div>' +
        (order.pickupType === 'curbside' && (Date.now() - order.createdAt) < 86400000 ?
          '<button onclick=\'sendArrivedFromHistory(' + JSON.stringify({orderId:order.orderId,customer:order.customer,carModel:order.carModel,carColor:order.carColor,pickupTime:order.pickupTime}) + ')\'' +
          ' style="width:100%;padding:13px;background:#27ae60;color:#fff;border:none;border-radius:10px;font-family:\'DM Sans\',sans-serif;font-size:13px;letter-spacing:0.08em;text-transform:uppercase;cursor:pointer;margin-bottom:8px;">' +
          '🚗 I Have Arrived!' +
          '</button>' : '') +
...
async function sendArrivedFromHistory(orderData) {
  ...
  try {
    ...
    if (btn) { btn.disabled = false; btn.textContent = '🚗 I Have Arrived!'; }
    showToastMsg('Error sending notification. Please call us.');
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Build menu tabs from cached categories config
  const _cachedCats = localStorage.getItem('prb_menu_categories');
  if (_cachedCats && typeof window.buildGuestMenuTabs === 'function') {
    window.buildGuestMenuTabs(JSON.parse(_cachedCats));
  }
  ...
  if (_initUser?.email) {
    ...
    import(fbUrl2).then(({ loadPointsFromFirebase, loadProfileFromFirebase }) => {
      ...
      // Load profile info (skip on payment return)
      const _isPaymentReturn = new URLSearchParams(window.location.search).get('payment') === 'success';
      if (!_isPaymentReturn) {
        loadProfileFromFirebase(_initUser.email).then(fbProfile => {
          if (fbProfile) {
