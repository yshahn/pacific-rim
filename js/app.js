...
function fillUserInfo() {
  ...
  // Show required indicators for guest users
  const isGuest = !user;
...
function goTo(id, pushState = true) {
  ...
  if (id === 'reservation' && !serviceStatus.reservations) {
    ...
    if (!banner) {
      ...
      banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:9999;background:#c0392b;color:#fff;padding:16px 20px;text-align:center;font-size:15px;font-weight:600;line-height:1.5;';
      banner.innerHTML = '⚠️ Online reservations are temporarily unavailable.<br>Please call us at <a href="tel:+14048930018" style="color:#fff;text-decoration:underline;">(404) 893-0018</a>';
      document.body.appendChild(banner);
    }
    banner.style.display = 'block';
  }
...
// ─────────────────────────────────
// NEW CHECKOUT FLOW: Contact Info → Register/Guest Choice → Checkout
// ─────────────────────────────────
...
// Toppings are cart entries whose name starts with "<mainName> + ".
...
function buildCartScreen() {
  ...
  if (!listEl) return; // screen not in DOM (shouldn't happen) — bail safely
  ...
  groups.forEach((item, gi) => {
    ...
    // Count how many of this exact group (same name + same toppings) exist, for the qty control
    const qty = item.qty;
...
// Remove a whole group (main item + its toppings) by name — used by My Cart screen
...
async function goToCheckout() {
  ...
  // Not logged in — show contact info step first
  document.getElementById('step-menu').style.display = 'none';
  document.getElementById('step-checkout').style.display = 'none';
  const mc = document.getElementById('step-cart');
  if (mc) mc.style.display = 'none';
  ...
  // Prefill from any previously saved profile (returning guest)
  const profile = loadProfile();
...
function continueAsGuest() {
  // Carry contact info into the checkout guest-info fields, then go to checkout
  proceedToCheckoutScreen();
}
...
async function quickRegister() {
  ...
  // Save to Firebase so Admin can see this member
  try {
    const fbUrl = window.location.origin + '/js/firebase-menu.js';
...
function proceedToCheckoutScreen() {
  ...
  // If we collected contact info this session (guest flow) and user is still
  // not logged in, carry that info into the checkout fields as well.
  const user = getUser();
  if (!user && _pendingContactInfo) {
    ...
    // Save as profile for next time (returning guest convenience)
...
function buildMenu(tab) {
  ...
  if (!list) return;
  // Clear list but preserve photo-hint
  Array.from(list.children).forEach(child => {
    if (child.id !== 'photo-hint') child.remove();
  });
  ...
  // Show photo hint only if tab has images (it lives inside menu-list, preserve it)
  const hint = document.getElementById('photo-hint');
  if (hint) hint.style.display = items.some(i => i.img) ? 'block' : 'none';
  // Clear menu list but keep hint element
...
function buildPickupSlots() {
  ...
  // unconditionally regardless of the clock.
  ...
  if (s.allowSchedule !== false) {
    ...
    if (isBreakTime || pastClosing) {
      // During break, or closed: remove ASAP, show placeholder for now
  ...
  if (pastClosing && !hasRealOption) {
    ...
    closedOpt.value = '';
    closedOpt.textContent = '🌙 Closed for online orders';
...
async function confirmReservation() {
  ...
  if (!serviceStatus.reservations) {
    alert('We are temporarily not accepting online reservations. Please call us.');
    return;
  }
...
function checkYearlyReset() {
  ...
  if (needsReset) {
    ...
    import(fbUrl).then(async ({ loadPointsFromFirebase, savePointsToFirebase }) => {
      ...
      user.points     = basePoints;     // points balance untouched by the reset itself
      user.tierPoints = 0;              // reset annual tier tracker only
...
async function startStripeCheckout() {
  ...
  // Guard against double-submission: if the Pay button is already showing
  // "Processing..." (a previous click already started a checkout session),
  // ignore this click instead of starting a second one. This closes the
  // brief window between a click and payBtn.disabled actually taking
  // effect further down, where a fast double-tap could slip through.
  const _existingPayBtn = document.getElementById('pay-btn');
  if (_existingPayBtn && _existingPayBtn.disabled) return;

  // If the guest already has a pending order from a checkout they started
  // recently (e.g. they clicked Pay, went to Stripe, then hit the browser
  // Back button instead of completing or cancelling), warn them before
  // starting a SECOND checkout session for what might be the same order —
  // this is the most likely way to end up with two real charges.
  try {
    const _startedAt = parseInt(sessionStorage.getItem('prb_checkout_started_at') || '0');
    if (_startedAt && (Date.now() - _startedAt) < 15 * 60 * 1000) {
      const proceed = confirm(
        "It looks like you may have already started paying for this order a few minutes ago.\n\n" +
        "If you already completed payment, please check your email for a confirmation before paying again to avoid being charged twice.\n\n" +
        "Continue with a new payment anyway?"
      );
      if (!proceed) return;
    }
  } catch(e) {}
  sessionStorage.setItem('prb_checkout_started_at', String(Date.now()));

  // Validate pickup time selected (break time, or closed for the day)
  const select = document.getElementById('pickup-time-select');
  if (select && (select.disabled || !select.value)) {
    alert('We are closed for online orders right now. Please check our hours or call us.');
    return;
  }
  ...
  if (!user) {
    ...
    const lastName  = (document.getElementById('gi-lastname')  || {}).value.trim();
    ...
    if (!firstName || !/[a-zA-Z]/.test(firstName)) {
      ...
      document.getElementById('gi-firstname')?.focus();
      return;
    }
    if (!email || !email.includes('@')) {
      alert('Please enter a valid email address to continue.');
      document.getElementById('gi-email')?.focus();
      return;
    }
    if (!phone || phone.replace(/\D/g,'').length < 10) {
      alert('Please enter a valid phone number to continue.');
      document.getElementById('gi-phone')?.focus();
      return;
    }
  }
  ...
  const custPhone = (document.getElementById('gi-phone') || {}).value || '';
  try {
    const res = await fetch('/api/checkout', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        items, customerEmail: email, customerName: (firstName + ' ' + lastName).trim(),
        usePoints, pointsDiscount: selectedPointsDiscount || 0, tip,
        // Full order details — the server saves a pending order from these
        // BEFORE redirecting to Stripe, so the webhook (api/stripe-webhook.js)
        // can confirm + notify the moment payment succeeds, regardless of
        // what happens to this browser tab afterward.
        customerPhone: custPhone,
        pickupTime: selectedPickupTime ? selectedPickupTime.label : 'ASAP',
        specialRequest, pickupType: pickupType || 'instore', carModel, carColor, smsConsent,
      }),
    });
    const data = await res.json();
    if (data.url) {
      // Kept for the success-screen's immediate UI display only (order
      // number, item list, etc.) — the webhook is what actually creates
      // and notifies the order now, so this is no longer load-bearing.
      localStorage.setItem('prb_pending_order', JSON.stringify({
        orderItems: cart.map(i => ({ name: i.name, price: i.price, emoji: i.emoji })),
        subtotal, tax, tip, total,
        pickupTime: selectedPickupTime ? selectedPickupTime.label : 'ASAP',
        customer: { name: (firstName + ' ' + lastName).trim(), email, phone: custPhone },
        orderId: data.orderId || Date.now().toString().slice(-6),
        smsConsent,
        specialRequest,
...
function checkPaymentResult() {
  ...
  if (params.get('payment') === 'success') {
    sessionStorage.removeItem('prb_checkout_started_at');
    cart = [];
    ...
    if (pendingOrder) {
      try {
        ...
        // Order creation + email/SMS notification now happen server-side
        // via api/stripe-webhook.js the moment Stripe confirms payment —
        // NOT here. This used to call /api/notify-order and
        // saveOrderToFirebase directly from the success screen, which only
        // ran if this browser tab successfully reached this point; the
        // webhook is a guaranteed backstop regardless of what happens to
        // the tab after paying. Calling notify-order here too would send
        // duplicate emails/texts for the (much more common) case where the
        // browser DOES make it back here fine.

        // 2 & 3. Update points using Firebase as the source of truth.
        // localStorage can be lost/reset during the Stripe redirect (especially on
        // mobile), so we re-fetch the latest points from Firebase right before
        // applying this order's earn/redeem, instead of trusting local state blindly.
        ...
        if (_email) {
          ...
          import(_fbUrl).then(async ({ loadPointsFromFirebase, savePointsToFirebase }) => {
            ...
            try {
              ...
              if (fbPts) {
                // Firebase is the trusted balance — always prefer it over local
            ...
            const _newTier   = _baseTier + _earned; // tierPoints only goes UP (annual earn tracker)
            ...
            // Re-sync localStorage with the corrected values (rebuilds the user object
            // even if it was lost, as long as we have the email)
            ...
            console.log('✅ Points updated! earned:', _earned, 'used:', _used, 'new total:', _newPoints);
            ...
            // Points are still updated here (client-side, tied to this
            // logged-in session) — the webhook doesn't know about loyalty
            // points, only order creation + notifications. Order saving
            // itself is no longer done here; api/stripe-webhook.js already
            // wrote the order via the Admin SDK the moment Stripe confirmed
            // payment, so duplicating that save here would create two
            // order records for one purchase.
            const _saveWithRetry = (attempt) => {
              savePointsToFirebase(_email, _newPoints, _newLog, _newTier)
                .then(ok => {
                  if (ok) {
                    console.log('✅ Points saved to Firebase:', _newPoints);
                  } else if (attempt < 3) {
          ...
          // No email available at all (truly anonymous guest) — still show a generic message
          const _earnedEl = document.getElementById('earned-pts');
          if (_earnedEl) _earnedEl.textContent = 'Sign in to earn points!';
          const _earnedBadge = document.querySelector('.earned-badge');
          if (_earnedBadge) _earnedBadge.style.display = 'block';
          // Order saving for guests also now happens via the webhook —
          // nothing to do here for the order itself.
        }
...
function loadTodaysPicks() {
  ...
  // Looks up an item's CURRENT price from the live menu by name AND
  // category, instead of trusting pick.origPrice — origPrice used to be a
  // one-time snapshot taken when the pick was set up in admin, so raising
  // the item's real menu price later (e.g. Edamame $6 → $7) never updated
  // the "original price" shown here. Matching on name alone isn't enough
  // either: the same name can exist in more than one category (a Lunch
  // "Edamame" vs a Dinner "Edamame" at a different price) — pick.cat
  // (saved by admin's updatePickItem) disambiguates which one this pick
  // actually refers to. Falls back to name-only for picks saved before
  // category tracking existed.
  function getCurrentMenuPrice(name, cat, fallback) {
    if (window.firebaseMenuData) {
      for (const [catKey, items] of Object.entries(window.firebaseMenuData)) {
        if (cat && catKey !== cat) continue;
        const found = (items || []).find(m => m.n === name);
        if (found && typeof found.p === 'number') return found.p;
      }
      if (cat) {
        for (const items of Object.values(window.firebaseMenuData)) {
          const found = (items || []).find(m => m.n === name);
          if (found && typeof found.p === 'number') return found.p;
        }
      }
  ...
  picks.forEach(pick => {
    const currentOrigPrice = getCurrentMenuPrice(pick.name, pick.cat, pick.origPrice);
    const hasDiscount = pick.salePrice !== null && pick.salePrice !== undefined && pick.salePrice < currentOrigPrice;
    const div = document.createElement('div');
    ...
    div.innerHTML =
      '<div>' +
        ...
        (hasDiscount
          ? '<div class="fi-price"><span class="fi-orig-price">$' + currentOrigPrice.toFixed(2) + '</span> <span class="fi-sale-price">$' + pick.salePrice.toFixed(2) + '</span></div>'
          : '<div class="fi-price">$' + currentOrigPrice.toFixed(2) + '</div>'
        ) +
      '</div>' +
    ...
    div.addEventListener('click', () => {
      addToCart(pick.name, pick.salePrice || currentOrigPrice, pick.emoji, null);
      goTo('order');
...
async function initServiceStatus() {
  try {
    const cached = JSON.parse(localStorage.getItem('prb_services_v2') || 'null');
  ...
  try {
    ...
    if (fresh) {
      ...
      localStorage.setItem('prb_services_v2', JSON.stringify(serviceStatus));
    }
