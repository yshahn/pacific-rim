...
async function startStripeCheckout() {
  ...
  // If curbside pickup is selected, vehicle info is required — without it,
  // staff have no way to identify the car when the order is ready, and the
  // confirmation still said "Curbside Pickup" even when these fields were
  // left blank because nothing actually checked for that before now.
  if (pickupType === 'curbside') {
    ...
    const carModel = carModelEl ? carModelEl.value.trim() : '';
    const carColor = carColorEl ? carColorEl.value.trim() : '';
    if (!carModel || !carColor) {
      alert('Please enter your car model and color for curbside pickup.');
      if (!carModel && carModelEl) carModelEl.focus();
      else if (carColorEl) carColorEl.focus();
      return;
    }
  }
    ...
    if (pendingOrder) {
      try {
        ...
        if (_email) {
          ...
          const _earnedBadge = document.querySelector('.earned-badge');
          if (_earnedBadge) _earnedBadge.style.display = 'block';
          // Save guest order to Firebase so it shows in Admin dashboard
          ...
          import(_fbUrl2).then(({ saveOrderToFirebase }) => {
            const _saveGuestOrderWithRetry = (attempt) => {
              saveOrderToFirebase(orderData).then(ok => {
                console.log('✅ Guest order saved to Firebase:', ok);
