...
function loadTodaysPicks() {
  ...
  // Looks up an item's CURRENT price from the live menu by name, instead
  // of trusting pick.origPrice — origPrice used to be a one-time snapshot
  // taken when the pick was set up in admin, so raising the item's real
  // menu price later (e.g. Edamame $6 → $7) never updated the "original
  // price" shown here, even though the sale price was correctly based on
  // the new price.
  function getCurrentMenuPrice(name, fallback) {
    if (window.firebaseMenuData) {
      for (const cat of Object.values(window.firebaseMenuData)) {
        const found = (cat || []).find(m => m.n === name);
  ...
  picks.forEach(pick => {
    const currentOrigPrice = getCurrentMenuPrice(pick.name, pick.origPrice);
