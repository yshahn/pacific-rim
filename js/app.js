...
function buildCheckoutSummary() {
  ...
  if (editList) {
    ...
    cart.forEach(item => {
      const mainName = Object.keys(mainItems).find(k => item.name.startsWith(k + ' + '));
      if (mainName) {
        if (!toppingMap[mainName]) toppingMap[mainName] = {};
        if (!toppingMap[mainName][item.name]) toppingMap[mainName][item.name] = { name:item.name, emoji:item.emoji, price:item.price, qty:0 };
        toppingMap[mainName][item.name].qty++;
      } else {
        if (!mainItems[item.name]) mainItems[item.name] = { ...item, qty:0 };
        mainItems[item.name].qty++;
      }
    });

    editList.innerHTML = '';
    Object.values(mainItems).forEach(item => {
      const toppings = Object.values(toppingMap[item.name] || {});
      const toppingHtml = toppings.map(t =>
        `<div style="display:flex;align-items:center;gap:6px;padding:3px 0 0 36px;font-size:12px;color:var(--muted);">
          <span>${t.emoji || '🧂'}</span>
          <span style="flex:1;">${t.name.replace(item.name + ' + ', '')}${t.qty > 1 ? ' ×'+t.qty : ''}</span>
          <span style="color:var(--gold);">+$${(t.price * t.qty).toFixed(2)}</span>
        </div>`
      ).join('');
      ...
      row.innerHTML = `
        ...
        <div class="cer-controls">
          <button class="cer-btn" onclick="cartQty('${item.name.replace(/'/g, "\\'")}', -1)">−</button>
          <span class="cer-qty">${item.qty}</span>
          <button class="cer-btn" onclick="cartQty('${item.name.replace(/'/g, "\\'")}', 1)">+</button>
          <button class="cer-del" onclick="cartRemove('${item.name.replace(/'/g, "\\'")}')">🗑</button>
        </div>
...
function buildMenu(tab) {
  ...
  items.forEach((item, idx) => {
    ...
    if (item.toppings && item.toppings.length) {
      // Toppings: + button opens topping modal
...
function closeToppingModal() {
  ...
  _toppingItem = null; _toppingQty = 1;
}
