...
function buildMenu(tab) {
  ...
  // Add section headers for nigiri tab
  if (tab === 'nigiri') {
    const nigiriHeader = document.createElement('div');
    nigiriHeader.className = 'menu-section-header';
    nigiriHeader.textContent = 'Nigiri Sushi';
    list.appendChild(nigiriHeader);
  }

  (menuData[tab] || []).forEach((item, idx) => {
    // Add Sashimi section header before first sashimi item
    if (tab === 'nigiri' && item.n === 'Salmon Sashimi (3pcs)') {
      const sashimiHeader = document.createElement('div');
      sashimiHeader.className = 'menu-section-header';
      sashimiHeader.textContent = 'Sashimi';
      list.appendChild(sashimiHeader);
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (tabs) {
    ...
    tabs.addEventListener('mousedown', e => {
      isDown = true; tabs.style.cursor = 'grabbing';
