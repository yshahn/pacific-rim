...
function applyGiftCard() {
  ...
  if (!gc || gc.code !== code) { showToastMsg('⚠️ Invalid gift card code'); return; }
  if (gc.balance <= 0) { showToastMsg('⚠️ Gift card has no remaining balance'); return; }
  window._appliedGiftCard = gc;
  showToastMsg('✅ Gift card applied! Balance: $' + gc.balance.toFixed(2));
  buildCheckoutSummary();
}
