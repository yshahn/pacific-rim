  ...
  if (params.get('payment') === 'success') {
    ...
    if (pendingOrder) {
      try {
        ...
        const _fbSaveUrl = window.location.origin + '/js/firebase-menu.js';
        import(_fbSaveUrl).then(({ saveOrderToFirebase }) => {
          saveOrderToFirebase(orderData).catch(e => console.warn('Order save failed:', e));
        }).catch(e => console.warn('Firebase import failed:', e));

        // 3. Update points - runs synchronously, no async dependencies
        const _user = getUser();
        if (_user) {
          const _earnRate = getEarnRate(_user);
          const _earned = Math.floor((orderData.subtotal || 0) * _earnRate);
          const _used = orderData.pointsDiscount ? orderData.pointsDiscount * 100 : 0;
          ...
          if (_used > 0) _user.pointsLog.push({ desc: 'Points redeemed ($' + orderData.pointsDiscount + ' off)', pts: -_used, date: Date.now() });
          _user.pointsLog.push({ desc: 'Order #' + (orderData.orderId || ''), pts: _earned, date: Date.now() });
          localStorage.setItem(USER_KEY, JSON.stringify(_user));
          console.log('🔍 Points updated! earned:', _earned, 'used:', _used, 'new total:', _user.points);
          document.getElementById('earned-pts').textContent = '+' + _earned + ' pts earned';
          const _earnedBadge = document.querySelector('.earned-badge');
          if (_earnedBadge) _earnedBadge.style.display = 'block';
          updateAuthUI();
          // Save to Firebase async
          import(_fbSaveUrl).then(({ savePointsToFirebase }) => {
            savePointsToFirebase(_user.email, _user.points, _user.pointsLog, _user.tierPoints).catch(() => {});
          }).catch(() => {});
        }
