...
function isLunchHours() {
  ...
  return mins >= 11 * 60 + 30 && mins < 14 * 60; // 11:30AM - 2:00PM
}
...
function buildPickupSlots() {
  ...
  if (s.allowSchedule !== false) {
    ...
    const windows = [
      { open: 11 * 60,      close: 14 * 60 + 30 },  // 11:00AM – 2:30PM
      { open: 17 * 60,      close: 21 * 60 + 15 },  // 5:00PM – 9:15PM
    ];

    // If current time is between 2:00PM and 5:00PM (break time),
    // skip ASAP and jump to 5:00PM as earliest scheduled slot
    const nowMins = now.getHours() * 60 + now.getMinutes();
    const isBreakTime = nowMins >= 14 * 60 && nowMins < 17 * 60;
    if (isBreakTime) {
      // Remove ASAP option and hide it — not available during break
      select.innerHTML = '';
    ...
    // Start from earliest or 5PM during break time
    let t = isBreakTime
      ? (() => { const d = new Date(now); d.setHours(17, 0, 0, 0); return d; })()
      : new Date(earliest.getTime() + 15 * 60 * 1000);
