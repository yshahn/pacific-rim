...
// ─────────────────────────────────
const FCM_VAPID_KEY = 'BPW-2ud6GRbO9gV5zdPorrlkHveBPCzuIaQIzrFuWgsIJC07GENuWr44ng1CclxJgnrdTinwwgLLJPvM-w-PRRE';
...
async function initPushNotifications() {
  try {
    ...
    if (token) {
      ...
      const user = getUser();
      const fbUrl = window.location.origin + '/js/firebase-menu.js';
      import(fbUrl).then(({ savePushTokenToFirebase }) => {
        savePushTokenToFirebase(token, user?.email || '', user?.email ? 'guest' : 'anonymous')
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => initPushNotifications(), 3000); // delay to not block initial load
