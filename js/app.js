...
async function confirmReservation() {
  ...
  try {
    if (window._saveReservationToFirebase) {
      await window._saveReservationToFirebase(reservation);
    }
  } catch(e) {
  ...
  if (params.get('payment') === 'success') {
    ...
    if (pendingOrder) {
      try {
        ...
        // Save order to Firebase for Admin
        if (window._saveOrderToFirebase) {
          window._saveOrderToFirebase(orderData)
