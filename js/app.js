...
function goTo(id) {
  ...
  if (id === 'reservation') { buildDates(); prefillReservationForm(); }
}
...
function buildCheckoutSummary() {
  ...
  const gc          = window._appliedGiftCard;
  const gcDiscount  = gc ? Math.min(gc.balance, subtotal + platformFee + tax + tip - discount) : 0;
  const total       = Math.max(subtotal + platformFee + tax + tip - discount - gcDiscount, 0);

  const tipAmountEl = document.getElementById('tip-amount');
  if (tipAmountEl) tipAmountEl.textContent = 'Tip: $' + tip.toFixed(2);

  // Update gift card applied info
  const gcInfo = document.getElementById('gc-applied-info');
  if (gcInfo && gc) {
    gcInfo.style.display = 'block';
    gcInfo.textContent = '✅ Gift card applied · Balance: $' + gc.balance.toFixed(2) + ' → −$' + gcDiscount.toFixed(2);
  }
  ...
  el.innerHTML =
    ...
    (gcDiscount ? '<div class="os-row os-label" style="color:var(--gold)"><span>🎴 Gift Card</span><span>−$' + gcDiscount.toFixed(2) + '</span></div>' : '') +
    '<div class="os-row total"><span>Total</span><span>$' + total.toFixed(2) + '</span></div>';

  const payBtn = document.getElementById('pay-btn');
  if (payBtn) {
    if (total === 0) {
      payBtn.textContent = 'Complete Order (Gift Card)';
    } else {
  ...
  let items = (window.firebaseMenuData && window.firebaseMenuData[tab])
    ? window.firebaseMenuData[tab].map(savedItem => {
        ...
        return builtIn ? {
          ...
          multiSelect: builtIn.multiSelect || null,
        } : savedItem;
      })
    : menuData[tab] || [];
...
async function confirmReservation() {
  ...
  const resPhone = document.getElementById('res-phone')?.value.trim() || user?.phone || profile.phone || '';
  const resEmail = document.getElementById('res-email')?.value.trim() || user?.email || profile.email || '';
...
function buildRewardsScreen() {
  ...
  document.querySelectorAll('.redeem-item').forEach(item => {
    ...
    if (points >= needed) { btn.classList.remove('locked'); btn.textContent = 'Use'; btn.onclick = () => goTo('order'); }
    else { btn.classList.add('locked'); btn.textContent = '🔒'; btn.onclick = null; }
  });
}
...
async function startStripeCheckout() {
  ...
  try {
    ...
    if (data.url) {
      ...
      const gcDiscount = window._appliedGiftCard
        ? Math.min(window._appliedGiftCard.balance, subtotal + platformFee + tax + tip - discount)
        : 0;
...
function checkPaymentResult() {
  // Check if this is a gift card purchase return
  if (checkGiftCardPurchase()) return;
    ...
    if (pendingOrder) {
      try {
        ...
        fetch('/api/notify-order', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({...orderData, notifEmails: getNotifEmails(), notifPhones: getNotifPhones(), guestSmsConsent: orderData.smsConsent}),
        ...
        if (orderData.gcDiscount && orderData.gcDiscount > 0) {
          const gc = loadGiftCard();
          if (gc) {
            gc.balance = Math.max(0, gc.balance - orderData.gcDiscount);
            gc.transactions = gc.transactions || [];
            gc.transactions.push({ date: Date.now(), amount: orderData.gcDiscount });
            saveGiftCard(gc);
          }
          window._appliedGiftCard = null;
        }
...
const GIFT_CARD_AMOUNTS = [
  ...
  { amount: 200, bonus: 11,  total: 222  },
  { amount: 300, bonus: 12,  total: 336  },
  { amount: 400, bonus: 13,  total: 452  },
  { amount: 500, bonus: 15,  total: 575  },
];

let selectedGiftAmount = null;

function showGiftCardScreen() {
  document.querySelectorAll('.screen').forEach(s => s.classList.remove('active'));
  document.getElementById('screen-giftcard').classList.add('active');
  const nav = document.getElementById('bottom-nav');
  if (nav) nav.style.display = 'none';
  buildGiftCardScreen();
}

function buildGiftCardScreen() {
  const gc = loadGiftCard();
  const balanceEl = document.getElementById('gc-balance');
  const codeEl    = document.getElementById('gc-code');
  const bonusEl   = document.getElementById('gc-bonus');
  if (gc && gc.balance > 0) {
    if (balanceEl) balanceEl.textContent = '$' + gc.balance.toFixed(2);
    if (codeEl)    codeEl.textContent    = gc.code;
    if (bonusEl)   bonusEl.textContent   = 'Bonus included: $' + (gc.totalPaid - gc.originalAmount).toFixed(2);
    document.getElementById('gc-has-card').style.display = 'block';
    document.getElementById('gc-no-card').style.display  = 'none';
  } else {
    document.getElementById('gc-has-card').style.display = 'none';
    document.getElementById('gc-no-card').style.display  = 'block';
  }
  const selector = document.getElementById('gc-amount-selector');
  if (selector) {
    selector.innerHTML = '';
    GIFT_CARD_AMOUNTS.forEach(opt => {
      const btn = document.createElement('div');
      btn.className = 'gc-amount-btn';
      btn.dataset.amount = opt.amount;
      btn.innerHTML =
        '<div style="display:flex;justify-content:space-between;align-items:center;">' +
          '<div>' +
            '<div style="font-family:Cormorant Garamond,serif;font-size:22px;font-weight:600;">$' + opt.amount + '</div>' +
            '<div style="font-size:11px;color:var(--muted);margin-top:2px;">You pay $' + opt.amount + '</div>' +
          '</div>' +
          '<div style="text-align:right;">' +
            '<div style="font-size:18px;font-weight:600;color:var(--gold);">$' + opt.total + ' value</div>' +
            '<div style="font-size:11px;color:var(--gold);">+' + opt.bonus + '% bonus</div>' +
          '</div>' +
        '</div>';
      btn.addEventListener('click', () => {
        selector.querySelectorAll('.gc-amount-btn').forEach(b => b.classList.remove('selected'));
        btn.classList.add('selected');
        selectedGiftAmount = opt;
        const buyBtn = document.getElementById('gc-buy-btn');
        if (buyBtn) {
          buyBtn.textContent = 'Purchase $' + opt.amount + ' → Get $' + opt.total + ' Value';
...
function loadGiftCard() {
  try { return JSON.parse(localStorage.getItem('prb_giftcard') || 'null'); }
  catch(e) { return null; }
}

function saveGiftCard(gc) {
  localStorage.setItem('prb_giftcard', JSON.stringify(gc));
}
...
async function purchaseGiftCard() {
  if (!selectedGiftAmount) { showToastMsg('Please select an amount'); return; }
  const user = getUser();
  if (!user?.email) { showToastMsg('Please sign in to purchase a gift card'); return; }
  const buyBtn = document.getElementById('gc-buy-btn');
  if (buyBtn) { buyBtn.textContent = 'Processing...'; buyBtn.disabled = true; }
  try {
    const res = await fetch('/api/checkout', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        items: [{ name: 'Pacific Rim Bistro Gift Card ($' + selectedGiftAmount.total + ' value)', price: selectedGiftAmount.amount, quantity: 1 }],
    ...
    const data = await res.json();
    if (data.url) {
      localStorage.setItem('prb_pending_giftcard', JSON.stringify({
        amount: selectedGiftAmount.amount,
        total:  selectedGiftAmount.total,
        bonus:  selectedGiftAmount.bonus,
        email:  user.email,
      }));
      window.location.href = data.url;
    ...
    if (buyBtn) { buyBtn.textContent = 'Purchase Gift Card'; buyBtn.disabled = false; }
  }
}

function checkGiftCardPurchase() {
  const params = new URLSearchParams(window.location.search);
  if (params.get('payment') !== 'success') return false;
  const pending = localStorage.getItem('prb_pending_giftcard');
  if (!pending) return false;
  try {
    const gc = JSON.parse(pending);
    const code = generateGiftCode();
    const giftCard = {
      code,
      balance: gc.total,
      originalAmount: gc.amount,
      totalPaid: gc.total,
      createdAt: Date.now(),
      email: gc.email,
      transactions: [],
    ...
    localStorage.removeItem('prb_pending_giftcard');
    history.replaceState({}, '', '/');
    document.querySelectorAll('.screen').forEach(s => s.classList.remove('active'));
    document.getElementById('success-screen').classList.add('active');
    document.getElementById('bottom-nav').style.display = 'none';
    document.getElementById('success-icon').textContent  = '🎁';
    document.getElementById('success-title').textContent = 'Gift Card Activated!';
    document.getElementById('success-msg').textContent   = 'Your gift card code: ' + code + ' | Balance: $' + gc.total.toFixed(2);
    document.getElementById('earned-pts').textContent    = '+$' + (gc.total - gc.amount).toFixed(2) + ' bonus!';
    return true;
...
function toggleGiftCard() {
  const gc = loadGiftCard();
  if (!gc || gc.balance <= 0) return;
  useGiftCard = !useGiftCard;
  const toggle = document.getElementById('gc-toggle');
  if (toggle) toggle.classList.toggle('on', useGiftCard);
  window._appliedGiftCard = useGiftCard ? gc : null;
  buildCheckoutSummary();
}

function initGiftCardCheckout() {
  const gc = loadGiftCard();
  const section = document.getElementById('gc-checkout-section');
  const label   = document.getElementById('gc-toggle-label');
  const sub     = document.getElementById('gc-toggle-sub');
  if (!section) return;
  if (gc && gc.balance > 0) {
    section.style.display = 'block';
    if (label) label.textContent = 'Use Gift Card';
    if (sub)   sub.textContent   = 'Balance: $' + gc.balance.toFixed(2);
  } else {
...
function applyGiftCard() {} // kept for compatibility
