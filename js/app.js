...
function goTo(id) {
  ...
  if (id === 'reservation') prefillReservationForm();
}
...
function buildPickupSlots() {
  ...
  // Earliest pickup = now + prep time, rounded up to next 15 min
  ...
  select.addEventListener('change', () => {
    selectedPickupTime = { label: select.value };
  });
...
async function confirmReservation() {
  ...
  // Save to Firebase via firebase-menu.js
  try {
    import('./js/firebase-menu.js').then(({ saveReservationToFirebase }) => {
      saveReservationToFirebase(reservation)
        .then(() => console.log('✅ Reservation saved to Firebase'))
        .catch(e => console.warn('Reservation save failed:', e));
    });
  } catch(e) {
    console.warn('Firebase reservation save failed:', e);
  }
  ...
  if (params.get('payment') === 'success') {
    ...
    if (pendingOrder) {
      try {
        ...
        // Save order to Firebase — import directly to avoid timing issues
        import('./js/firebase-menu.js').then(({ saveOrderToFirebase }) => {
