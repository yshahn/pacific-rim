...
function buildMenu(tab) {
  ...
  items.forEach((item, idx) => {
    ...
    // Toppings — shown as bottom modal when + is tapped
    const toppingsHtml = '';  // No inline toppings — handled via openToppingModal()
    ...
    if (item.multiSelect) {
      ...
      // Toppings: + button opens topping selection modal
      const addBtn = div.querySelector('.mi-add');
      const qtyCtrl = div.querySelector('.mi-qty-ctrl');
...
function openToppingModal(item) {
  ...
  // When any checkbox is checked, uncheck "No Topping" radio
  body.querySelectorAll('input[type="checkbox"]').forEach(chk => {
    chk.addEventListener('change', () => {
      ...
      // If all unchecked, re-check "No Topping"
  ...
  // When "No Topping" selected, uncheck all toppings
