...
function buildMenu(tab) {
    ...
    ? window.firebaseMenuData[tab].map(savedItem => {
        ...
        if (builtIn) {
          return {
            ...
            toppings:    savedItem.toppings || null,
          };
        }
        return { ...savedItem, multiSelect: fixMultiSelect(savedItem.multiSelect), toppings: savedItem.toppings || null };
      })
    : menuData[tab] || [];
  ...
  items.forEach((item, idx) => {
    ...
    // Toppings — multi-select checkboxes shown below item
    const toppingsHtml = (item.toppings && item.toppings.length) ? `
      <div class="mi-toppings" style="display:none;padding:8px 12px 12px;border-top:1px solid #f0ebe2;background:#fafaf8;">
        <div style="font-size:10px;font-weight:600;color:var(--muted);letter-spacing:0.1em;text-transform:uppercase;margin-bottom:8px;">🧂 Add Toppings</div>
        ${item.toppings.map((t, ti) => {
          const inCart = cart.filter(c => c.name === item.n + ' + ' + t.n).length > 0;
          return `<div class="mi-topping-row" data-idx="${ti}" data-name="${item.n} + ${t.n}" data-price="${t.p || 0}" data-emoji="${t.e || '🧂'}">
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:5px 0;border-bottom:1px solid #f5f0e8;">
              <input type="checkbox" class="mi-topping-check" data-idx="${ti}" ${inCart ? 'checked' : ''}
                style="width:18px;height:18px;accent-color:var(--gold);cursor:pointer;flex-shrink:0;"/>
              <span style="font-size:16px;">${t.e || '🧂'}</span>
              <span style="flex:1;font-size:13px;">${t.n}</span>
              <span style="font-size:13px;font-weight:600;color:var(--gold);">${parseFloat(t.p) > 0 ? '+$' + parseFloat(t.p).toFixed(2) : 'Free'}</span>
            </label>
          </div>`;
        }).join('')}
        <button class="mi-toppings-add-btn" style="width:100%;margin-top:10px;padding:10px 0;background:var(--gold);color:var(--ink);border:none;border-radius:10px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer;">Add to Cart</button>
      </div>` : '';
    ...
    } else if (item.toppings && item.toppings.length) {
      // Toppings expand button
      div.querySelector('.mi-expand-btn').addEventListener('click', function() {
        const toppingsEl = div.querySelector('.mi-toppings');
        const isOpen = toppingsEl.style.display === 'block';
        toppingsEl.style.display = isOpen ? 'none' : 'block';
        this.textContent = isOpen ? '▾' : '▴';
        this.classList.toggle('active', !isOpen);
      });
      // Add to Cart button — adds base item + selected toppings
      const addBtn = div.querySelector('.mi-toppings-add-btn');
      if (addBtn) {
        addBtn.addEventListener('click', () => {
          const checks = div.querySelectorAll('.mi-topping-check');
          // Always add base item
          addToCart(item.n, parseFloat(item.p), item.e, null);
          // Add each checked topping
          checks.forEach((chk, ti) => {
            if (chk.checked) {
              const t = item.toppings[ti];
              if (t) addToCart(item.n + ' + ' + t.n, parseFloat(t.p) || 0, t.e || '🧂', null);
            }
          });
          // Close toppings panel
          const toppingsEl = div.querySelector('.mi-toppings');
          if (toppingsEl) toppingsEl.style.display = 'none';
          const expBtn = div.querySelector('.mi-expand-btn');
          if (expBtn) { expBtn.textContent = '▾'; expBtn.classList.remove('active'); }
          updateCartBar();
...
window.buildGuestMenuTabs = function buildGuestMenuTabs(cats) {
  ...
  // Trigger first tab
  const firstTab = container.querySelector('.menu-tab.active');
  if (firstTab) {
    const firstKey = cats[0].key;
    buildMenu(firstKey);
    const notice = document.getElementById('lunch-notice');
    if (notice) notice.style.display = firstKey === 'lunch' ? 'block' : 'none';
  }
}
