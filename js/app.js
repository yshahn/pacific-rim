...
function buildMenu(tab) {
  ...
  items.forEach((item, idx) => {
    ...
    // Toppings — always visible below item (no expand needed)
    const toppingsHtml = (item.toppings && item.toppings.length) ? `
      <div class="mi-toppings" style="padding:8px 12px 12px;border-top:1px solid #f0ebe2;background:#fafaf8;">
        <div style="font-size:10px;font-weight:600;color:var(--muted);letter-spacing:0.1em;text-transform:uppercase;margin-bottom:6px;">🧂 Add Toppings</div>
    ...
    if (item.multiSelect) {
      ...
      // Toppings are always visible — just handle Add to Cart button
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Set initial state so back button has somewhere to go
  history.replaceState({ screen: 'home' }, '', window.location.pathname);

  window.addEventListener('popstate', (e) => {
    const screen = e.state?.screen || 'home';
    // Navigate back without pushing new state
    goTo(screen, false);
    // If going back to home from order, clear cart UI
    if (screen === 'home') {
      const nav = document.getElementById('bottom-nav');
      if (nav) nav.style.display = 'flex';
    }
