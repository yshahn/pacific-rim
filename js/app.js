...
function buildHomeScreen() {
  const user = getUser();
  const bannerEl = document.getElementById('home-bottom-banner');
  if (!bannerEl) return;
  if (user && user.email) {
    const pts = (user.points || 0).toLocaleString();
    const tier = getTier(user.tierPoints || user.points || 0);
    bannerEl.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;">
        <div>
          <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:0.05em;text-transform:uppercase;">My Rewards</div>
          <div style="font-size:22px;font-weight:700;color:var(--gold);">${pts} pts</div>
          <div style="font-size:11px;color:rgba(255,255,255,0.5);">${tier.icon} ${tier.name} · ${tier.rate}% earn rate</div>
        </div>
        <button onclick="goTo('rewards')" style="background:var(--gold);color:var(--ink);border:none;border-radius:10px;padding:10px 18px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer;">View Rewards →</button>
      </div>`;
    ...
    bannerEl.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;">
        <div>
          <div style="font-size:14px;font-weight:600;color:var(--gold);">Join Our Rewards Program</div>
          <div style="font-size:11px;color:rgba(255,255,255,0.5);margin-top:2px;">Earn points on every order · Get 200 pts free</div>
        </div>
        <a href="/login.html" style="background:var(--gold);color:var(--ink);border:none;border-radius:10px;padding:10px 18px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer;text-decoration:none;">Sign Up →</a>
      </div>`;
    bannerEl.style.display = 'block';
  }
}
    ...
    import(fbUrl).then(({ loadOrdersFromFirebase }) => {
      loadOrdersFromFirebase(20).then(orders => {
        ...
        histEl.innerHTML = myOrders.map(o => {
          ...
          const items = (o.orderItems || o.items || []).map(i => i.name || i.n).join(', ');
          ...
          return '<div style="padding:14px 16px;border-bottom:1px solid var(--border);">' +
            ...
            '</div>' +
            '<div style="font-size:12px;color:var(--muted);margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + items + '</div>' +
          '</div>';
        }).join('');
