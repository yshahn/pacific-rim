      ...
      if (isFirstOfSection) {
        ...
        accHeader.innerHTML =
          '<div class="mi-main" style="width:100%;pointer-events:none;">' +
            ...
            '</div>' +
            '<div style="width:32px;height:32px;border-radius:50%;background:var(--ink);color:var(--gold);font-size:20px;font-weight:600;display:flex;align-items:center;justify-content:center;flex-shrink:0;">' + (isOpen ? '−' : '+') + '</div>' +
          '</div>';
...
const TIERS = [
  { name:'Bronze',   icon:'🥉', min:0,    max:499,  rate:2,   color:'#cd7f32' },
  { name:'Silver',   icon:'🥈', min:500,  max:999,  rate:2.5, color:'#a8a9ad' },
  { name:'Gold',     icon:'🥇', min:1000, max:1999, rate:3,   color:'#c8a96e' },
  { name:'Platinum', icon:'💎', min:2000, max:99999,rate:4,   color:'#5c6bc0' },
];
...
function getEarnRate(points) {
  return getTier(points).rate;
}

// Check yearly reset (Jan 1)
function checkYearlyReset() {
  ...
  if (isNewYear && now.getMonth() === 0 && now.getDate() === 1) {
    ...
    user.pointsLog.push({ desc: 'Annual Reset', pts: -(user.points || 0), date: Date.now() });
...
function buildRewardsScreen() {
  ...
  if (tiersEl) {
    tiersEl.innerHTML = TIERS.map(t => {
      ...
      const pct = t.name === 'Platinum' ? 100
        : Math.min(100, ((points - t.min) / (t.max - t.min + 1)) * 100);
      return '<div style="background:' + (isActive ? 'var(--ink)' : 'var(--card-bg)') + ';border:1px solid ' + (isActive ? 'var(--gold)' : 'var(--border)') + ';border-radius:12px;padding:14px 16px;margin-bottom:8px;">' +
        ...
        '</div>' +
        (isActive && nextTier ? '<div style="background:rgba(255,255,255,0.1);border-radius:4px;height:4px;overflow:hidden;"><div style="width:' + pct + '%;height:100%;background:var(--gold);border-radius:4px;"></div></div><div style="font-size:10px;color:rgba(255,255,255,0.5);margin-top:4px;">' + (nextTier.min - points) + ' pts to ' + nextTier.name + '</div>' : '') +
      '</div>';
    }).join('');
  }
  ...
  if (resetEl) {
    ...
    resetEl.textContent = '⏱ Points reset on Jan 1 · ' + daysLeft + ' days remaining';
  }
