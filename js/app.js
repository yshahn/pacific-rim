...
function buildMenu(tab) {
  ...
  try {
    ...
    if (saved) {
      ...
      if (savedMenu[tab] && savedMenu[tab].length > 0) items = savedMenu[tab];
    }
  } catch(e) {}

  // Fall back to built-in menuData (always has options)
  if (!items) items = menuData[tab] || [];
  // Merge options from built-in if missing in saved data
  else {
...
function initMenuStorage() {
  const MENU_VERSION = 'v3'; // bump this when menu structure changes
  const savedVersion = localStorage.getItem('prb_menu_version');

  if (savedVersion !== MENU_VERSION) {
    // New version — clear old cache and re-save
    localStorage.removeItem('prb_menu_data');
    localStorage.setItem('prb_menu_version', MENU_VERSION);
  }
