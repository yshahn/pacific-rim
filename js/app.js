...
function fillUserInfo() {
  ...
  const data    = { ...profile, ...(user || {}) }; // profile as base, user overrides
...
function buildMenu(tab) {
  ...
  // Always use built-in menuData (options always included)
  // Firebase/Admin overrides are applied via window.firebaseMenuData if available
  ...
  items.forEach((item, idx) => {
    ...
    if (item.multiSelect) {
      ...
      if (addBtn) {
        addBtn.addEventListener('click', () => openMultiSelectModal(item));
      }
      if (qtyCtrl) {
        qtyCtrl.querySelector('.mi-qty-plus').addEventListener('click', () => {
          openMultiSelectModal(item);
        });
        qtyCtrl.querySelector('.mi-qty-minus').addEventListener('click', () => {
          // Remove last matching item from cart
...
function updateOptRowQty(row, name, price, emoji) {
  ...
  if (ctrl) {
    ...
    if (count === 0) {
      // Replace qty ctrl back with + btn
...
function isLunchHours() {
  return true; // ── TEMP: disabled during development
}
...
function addToCart(name, price, emoji, btn) {
  ...
  // Update button (only if btn provided)
  if (btn) {
    const count = cart.filter(c => c.name === name).length;
...
function saveProfile() {
  ...
  // Also update prb_user for checkout autofill
...
function updateProfileUI() {
  const user    = getUser();
  const profile = loadProfile();
  // Merge: profile values take priority, fall back to user (login) data
  const data = {
...
function buildPickupSlots() {
  ...
  // Build ASAP slot
  const slots = [];
  const asapLabel = 'ASAP (~' + s.prepTime + ' min)';
  slots.push({ label: asapLabel, time: earliest, isAsap: true });

  // Build scheduled slots (every 15 min up to maxAdvance hours)
  if (s.allowSchedule !== false) {
    ...
    while (t <= closeTime && t.getTime() - now.getTime() <= maxMs) {
      slots.push({ label: t.toLocaleTimeString('en-US', { hour:'numeric', minute:'2-digit' }), time: new Date(t) });
  ...
  slots.forEach((slot, i) => {
    ...
    btn.className = 'pickup-slot' + (i === 0 ? ' selected' : '');
    btn.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:center;">' +
        '<div>' +
          '<div style="font-size:14px;font-weight:500;">' + slot.label + '</div>' +
          (slot.isAsap ? '' : '<div style="font-size:11px;color:var(--muted);">Scheduled pickup</div>') +
        '</div>' +
        '<div style="color:var(--gold);font-size:18px;">' + (i === 0 ? '✓' : '') + '</div>' +
      '</div>';
    btn.addEventListener('click', () => {
      container.querySelectorAll('.pickup-slot').forEach(b => {
        b.classList.remove('selected');
        b.querySelector('div > div:last-child').textContent = '';
      });
      btn.classList.add('selected');
      btn.querySelector('div > div:last-child').textContent = '✓';
      selectedPickupTime = slot;
    });
    container.appendChild(btn);
    if (i === 0) selectedPickupTime = slot;
  });
}
...
async function startStripeCheckout() {
  ...
  try {
    ...
    if (data.url) {
      // Save order info for notification after redirect
  ...
  if (params.get('payment') === 'success') {
    ...
    if (pendingOrder) {
      try {
        ...
        // Update success message with pickup time
...
function openMultiSelectModal(item) {
  ...
  item.multiSelect.forEach((group, gi) => {
    ...
    section.innerHTML = `
      <div style="font-size:12px;font-weight:600;color:var(--ink);margin-bottom:4px;">${group.label}
        <span style="font-size:11px;color:var(--muted);font-weight:400;"> · Choose up to ${group.max} · ${group.required ? 'Required' : 'Optional'}</span>
      </div>`;
...
function updateMsAddBtn() {
  ...
  const allRequired = _msItem.multiSelect.every((g, i) =>
    !g.required || _msSelections[i].length >= Math.min(g.max, 1)
  );
...
function initMenuStorage() {
  // Always clear stale localStorage — Firebase or built-in is the source of truth
