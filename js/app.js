  ...
  if (params.get('payment') === 'success') {
    ...
    if (pendingOrder) {
      try {
        ...
        // 2. Save order to Firebase (non-blocking)
        const fbUrl = window.location.origin + '/js/firebase-menu.js';
        ...
        // 3. Update points - deduct used, add earned (runs independently)
        try {
          const user = getUser();
          if (user) {
        ...
        } catch(e) { console.error('Points update error:', e.message, e.stack); }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load profile + points from Firebase (skip on payment return to avoid overwriting fresh points)
  const _initUser = getUser();
  if (_initUser?.email && !_isPaymentReturn) {
