...
const menuData = {
  lunch: [
    ...
    { e:'🍛', n:'Thai Yellow Curry (Bento)', d:'Potato, carrot, cashew nut, cucumber pickle · Served with Miso Soup, House Salad, Spring Roll and Steamed Rice', p:18, img:null,
      options:[{n:'Shrimp $18',p:18},{n:'Chicken $17',p:17},{n:'Tofu $16',p:16}] },
    { e:'🥑', n:'Green Avocado Curry (Bento)', d:'Asparagus, bell peppers, basil leaf & avocado · Served with Miso Soup, House Salad, Spring Roll and Steamed Rice', p:18, img:null,
      options:[{n:'Shrimp $18',p:18},{n:'Chicken $17',p:17},{n:'Tofu $16',p:16}] },
    { e:'🍛', n:'Panang Curry (Bento)', d:'Red and green bell peppers, asparagus, Thai basil leaves · Served with Miso Soup, House Salad, Spring Roll and Steamed Rice', p:18, img:null,
      options:[{n:'Shrimp $18',p:18},{n:'Chicken $17',p:17},{n:'Tofu $16',p:16}] },
    { e:'🌿', n:'Spicy Thai Basil (Bento)', d:'Bell peppers, onions & Thai basil leaves · Served with Miso Soup, House Salad, Spring Roll and Steamed Rice', p:18, img:null,
      options:[{n:'Shrimp $18',p:18},{n:'Chicken $17',p:17},{n:'Tofu $16',p:16}] },
...
function buildMenu(tab) {
  ...
  items.forEach((item, idx) => {
    ...
    if (tab === 'lunch') {
      ...
      // Render accordion header for first Bento item
      if (item.n === 'Sweet and Sour Fish') {
        const isOpen = window._lunchAccordion['bento'] || false;
        ...
        accHeader.innerHTML =
          '<div>' +
            '<div style="font-family:Cormorant Garamond,serif;font-size:18px;font-weight:500;">Bento Box Specials</div>' +
            '<div style="font-size:11px;color:var(--muted);margin-top:2px;">Served with Miso Soup, House Salad, Spring Roll and Steamed Rice</div>' +
          '</div>' +
          '<div style="font-size:20px;color:var(--gold);">' + (isOpen ? '−' : '+') + '</div>';
        accHeader.addEventListener('click', () => {
          window._lunchAccordion['bento'] = !window._lunchAccordion['bento'];
          buildMenu('lunch');
        });
        list.appendChild(accHeader);
        // Skip item if closed
        if (!isOpen) return;
      } else if (BENTO_ITEMS.includes(item.n) && !window._lunchAccordion['bento']) {
        return; // hide bento items when collapsed
      }
    }
