...
// ── FCM: Save push token to Firestore
export async function savePushTokenToFirebase(token, email, role) {
  try {
    const key = email ? email.replace(/[.@]/g, '_') : 'admin_' + Date.now();
    const tokenDoc = doc(db, 'push_tokens', key);
    await setDoc(tokenDoc, {
      token, email: email || '', role: role || 'guest',
      updatedAt: Date.now()
    }, { merge: true });
    return true;
  } catch(e) { console.error('FCM token save error:', e); return false; }
}

// ── FCM: Load all push tokens from Firestore
export async function loadPushTokensFromFirebase(role) {
  try {
    const tokensCol = collection(db, 'push_tokens');
    const snap = await getDocs(tokensCol);
    const all = snap.docs.map(d => d.data());
    return role ? all.filter(t => t.role === role) : all;
  } catch(e) { console.error('FCM tokens load error:', e); return []; }
}
