...
export async function saveMenuToFirebase(menuByCategory, version) {
  try {
    ...
    // 2. Auto-backup — save to menu_backups collection (keep last 10)
    try {
      ...
      await addDoc(backupsCol, {
        ...
        savedAt: now,
        itemCount: totalItems,
        label: new Date(now).toLocaleString('en-US', { month:'short', day:'numeric', hour:'numeric', minute:'2-digit' }),
      });

      // Trim to keep only latest 10 backups
      const q = query(backupsCol, orderBy('savedAt', 'desc'), limit(20));
      ...
      console.warn('Backup save failed (non-critical):', backupErr);
    }

    console.log(`✅ Menu saved & backed up: ${totalItems} items`);
    return true;
  } catch(e) {
