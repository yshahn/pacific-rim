...
import { getFirestore, doc, getDoc, setDoc, deleteDoc, onSnapshot, collection, addDoc, getDocs, orderBy, query, limit }
...
export async function loadMenuBackups() {
  ...
  const data = d.data();
  return { ...data, firebaseId: d.id, id: d.id };
});
  } catch(e) {
...
export async function loadAllMembersFromFirebase() {
  try {
    ...
    return snap.docs.map(d => ({ ...d.data(), docId: d.id, id: d.id }));
  } catch(e) {
...
// Each browser/device that enables push gets its own FCM token, saved here so
// the server can send to all of them at once (a device_id lets us replace a
// stale token instead of piling up duplicates when the same device re-enables).
export async function saveAdminPushToken(token, deviceLabel) {
  try {
    const tokenDoc = doc(db, 'adminPushTokens', token);
    await setDoc(tokenDoc, { token, deviceLabel: deviceLabel || 'Unknown device', updatedAt: Date.now() });
    return true;
  } catch(e) { console.error('Push token save error:', e); return false; }
}

export async function removeAdminPushToken(token) {
  try {
    await deleteDoc(doc(db, 'adminPushTokens', token));
    return true;
  } catch(e) { console.error('Push token remove error:', e); return false; }
}
